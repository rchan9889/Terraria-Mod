...
{
    public class GoldenInfernoDeckProjectile : ModProjectile
    {
        ...
        {  
            ...
            if (Projectile.owner == Main.myPlayer && Projectile.timeLeft <= 3) {
				...
				if (Main.rand.NextBool()) {
					...
					dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.IchorTorch, 0f, 0f, 100, default, 1f);
        ...
        public override void OnKill(int timeLeft)
        {
            if(Projectile.damage != 0){
                ...
                for (int i = 0; i < 80; i++) {
				    Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.IchorTorch, 0f, 0f, 100, default, 3f);
				    dust.noGravity = true;
				    dust.velocity *= 5f;
